...
def get_tiktok_count():
    try:
        conn = get_connection()
        with conn.cursor() as cursor:
            cursor.execute("SELECT COUNT(*) FROM TREND_CONTENT WHERE PLATFORM_TYPE = 'TIKTOK'")
...
def run_update():
    print(f"\n🔄 [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 좋아요/조회수 업데이트 시작")
