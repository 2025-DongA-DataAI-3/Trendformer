...
const Login = () => {
  ...
  const buildUserData = (data) => {
    const rawUser = data?.user || data?.result || data?.data || {};

    return {
      id: rawUser.id || form.id.trim(),
      nickname: rawUser.nickname || rawUser.nick || form.id.trim(),
      email: rawUser.email || `${form.id.trim()}@trendformer.com`,
      bio:
        rawUser.bio ||
        "실시간 밈과 숏폼 트렌드를 탐색하고 저장하는 TrendFormer 사용자입니다.",
      profileImage: rawUser.profileImage || "",
      uploadCount: Number(rawUser.uploadCount) || 0,
    };
  };
      ...
      <div className="login-content">
        <div className="login-floating-card">
          ...
          <form className="login-form" onSubmit={handleLogin}>
            ...
            {errors.common && (
              <div className="login-error-box">{errors.common}</div>
            )}
